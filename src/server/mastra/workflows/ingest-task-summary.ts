      ...
      try {
        const response = await agent.generate(
          [
            ...
            "5. 'agentDocuments': Propose updates to AGENT-LEVEL documentation only.",
            "   STRICT RULES for agentDocuments:",
            "   - ONLY propose changes to the canonical top-level agent files: CLAUDE.md, GEMINI.md, AGENTS.md, CODEX.md.",
            "   - DO NOT create per-function, per-module, per-component, or per-feature markdown files (e.g. NO 'auth-flow.md', 'database-schema.md', 'login-component.md').",
            "   - When updating an agent file, CONSOLIDATE related content into one cohesive document — do not append a new section for every small change.",
            "   - If a topic doesn't warrant agent-file changes, leave agentDocuments EMPTY rather than inventing a new file.",
            "   - Prefer UPDATING existing agent files (referenced in the snapshot) over creating new ones.",
            "6. 'skills': Propose additions or modifications to skills if there are skills impacts. Only suggest a skill when the work demonstrates a reusable pattern worth codifying — not for one-off bug fixes.",
            "",
            ...
            if (jsonStart !== -1) {
              ...
              if (parsed && typeof parsed === "object") {
                draft = {
                  ...
                  categories: Array.isArray(parsed.categories) ? parsed.categories.filter((c: any) => typeof c === 'string') : fallbackDraft.categories,
                  tags: Array.isArray(parsed.tags) ? parsed.tags.filter((t: any) => typeof t === 'string') : fallbackDraft.tags,
                  memories: Array.isArray(parsed.memories) ? parsed.memories.filter((m: any) => typeof m === 'object' && m !== null && m.fact && m.category) : fallbackDraft.memories,
                  todos: Array.isArray(parsed.todos) ? parsed.todos.filter((t: any) => typeof t === 'string') : fallbackDraft.todos,
                  agentDocuments: Array.isArray(parsed.agentDocuments) ? parsed.agentDocuments.filter((d: any) => typeof d === 'object' && d !== null && d.filename && d.content).map((d: any) => ({ ...d, reason: d.reason || "Updated document based on task." })) : fallbackDraft.agentDocuments,
                  skills: Array.isArray(parsed.skills) ? parsed.skills.filter((s: any) => typeof s === 'object' && s !== null && s.path && s.content).map((s: any) => ({ ...s, reason: s.reason || "Updated skill based on task.", action: s.action || "update" })) : fallbackDraft.skills,
                };
                usedFallback = false;
              }
            }
